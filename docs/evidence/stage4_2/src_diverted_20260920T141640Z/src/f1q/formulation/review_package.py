...
def _is_excluded(rel: Path) -> bool:
    ...
    if "evidence/ledger" in str(rel).replace("\\", "/"):
        return True
    # Final external reports written after ZIP freeze.
    if rel.as_posix() in {
...
def default_allowlist(root: Path) -> list[str]:
    ...
    # Prior receipts / artifacts required for preservation claims
    ...
    for run_id in (
        ...
        matrix_dir = root / f"evidence/formulation/artifacts/{run_id}/formulation.development_matrix"
        if matrix_dir.is_dir():
            for fp in sorted(matrix_dir.glob("*.record.json")):
                receipt_globs.append(fp.relative_to(root).as_posix())
        for name in ("receipts", "snapshots"):
            for ext in (".json", ".md"):
                p = root / f"evidence/formulation/{name}/{run_id}{ext}"
                if p.is_file():
                    receipt_globs.append(p.relative_to(root).as_posix())
    # All existing development specs
    for fp in sorted((root / "evidence/development").rglob("*.spec.json")):
        receipt_globs.append(fp.relative_to(root).as_posix())
...
def build_and_verify_review_package(
    ...
    *,
    ...
    prepackage_name: str | None = None,
    generation_command: str | None = None,
) -> dict[str, Any]:
    stem = zip_name.replace(".zip", "")
    staging = root / "review" / (staging_name or f"_{stem.lower()}_staging")
    ...
    pre = staging / "docs" / (prepackage_name or f"{stem}_PREPACKAGE_REPORT.md")
    if not pre.parent.exists():
        pre.parent.mkdir(parents=True, exist_ok=True)
    pre.write_text(
        f"# {stem} prepackage report\n\nInternal staging snapshot before ZIP freeze. Not the final external report.\n",
        encoding="utf-8",
    )
    rows = [
        canonical_row(p.relative_to(staging).as_posix(), p.read_bytes())
    ...
    sidecar_path = root / "review" / (sidecar_name or f"{stem}.manifest.json")
    sidecar_sha = write_external_sidecar(
        ...
        generation_command=generation_command or "python -m f1q.formulation.review_package",
    )
    # Re-check frozen ZIP hash immediately
    recheck = sha256_file(zip_path)
    extract_dir = Path(tempfile.mkdtemp(prefix=f"f1q_{stem.lower()}_extract_"))
