    ...
    *,
    ...
    if finished:
        ...
        if not commitment:
            ...
            # transit_out / service_already_completed uses the already-mounted set (may be marked used).
            inv_match = next((i for i in inventory if i.get("set_id") == commitment["set_id"]), None)
            if inv_match is None and not commitment.get("service_already_completed"):
                reject(cont, "committed_set_not_in_inventory")
            elif (
                inv_match is not None
                and inv_match.get("compound") != commitment.get("target_compound")
            ):
                reject(cont, "committed_compound_conflicts_inventory")
            elif (
                commitment.get("service_already_completed")
                and commitment.get("target_compound") != car.get("compound")
            ):
                reject(cont, "committed_mount_conflicts_current_compound")
            else:
                admit(cont)
