...
def project_committed_pit_service(
    *,
    ...
    model = CommittedPitService(
        ...
        pit_entry_completed_laps=(
            int(car["pit_entry_completed_laps"]) if car.get("pit_entry_completed_laps") is not None else None
        ),
        pit_parts_s={"t_in_s": t_in, "t_service_s": t_service, "t_out_s": t_out},
        availability_time=f"race_s:{t}",
        provenance={
            "constructed_by": "stage4_2_observation_commitment",
...
def service_interval_from_commitment(commitment: dict[str, Any] | None) -> tuple[float, float] | None:
    ...
    if phase == "transit_out":
        return None  # service already complete
    if phase == "service":
        start = 0.0
        end = remain_service
    elif phase == "waiting":
